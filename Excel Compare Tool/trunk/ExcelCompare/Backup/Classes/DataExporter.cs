    ...
    public class DataExporter
    {
        ...
        public void ExportToXLS(CompareTablesResult result, string fullNameFile)
        {
            ...
            finally
            {
                ...
                ExcelConverter.ReleaseCOMObj(workSheetA);
                ExcelConverter.ReleaseCOMObj(workSheetB);
                ExcelConverter.ReleaseCOMObj(workbook);
                ExcelConverter.ReleaseCOMObj(excelApp);
            }
        ...
        private void PlushDataToSheet(System.Data.DataTable table, Worksheet workSheet)
        {
            ...
            //    //foreach (DataColumn col in table.Columns)
            //    //{
            //        workSheet.get_Range(this.ProcessingRowIndex, table.Columns.Count+1).Value2 = row.ItemArray;
            //    //}
            //    this.ProcessingRowIndex++;
            //    //colunmIndex = 1;
            //}

            object[,] data = new object[table.Rows.Count + 1, table.Columns.Count];
            int rowIndex =0;
            int colIndex=0;

            foreach (DataColumn col in table.Columns)
            {
                data[rowIndex, colIndex++] = col.ColumnName;
            ...
            workSheet.get_Range(workSheet.Cells[1, 1], workSheet.Cells[rowIndex, colIndex]).Value2 = data;

            ((Range)workSheet.Rows[1,Type.Missing]).Font.Bold = true;
            ((Range)workSheet.Rows[1, Type.Missing]).Font.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Blue); ;
        }
        #endregion
